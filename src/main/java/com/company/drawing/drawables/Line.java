...
public class Line implements Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (x1 == x2) //vertical line
        {
            ...
            for (int y = Math.max(1, Math.min(y1, y2)); y <= Math.min(canvas.getHeight(), Math.max(y1, y2)); ++y)
                canvas.setPixel(x1, y);
        }
        else if (y1 == y2) //horizontal line
        {
            if (y1 < 1 || y1 > canvas.getHeight()) return;
            for (int x = Math.max(1, Math.min(x1, x2)); x <= Math.min(canvas.getWidth(), Math.max(x1, x2)); ++x)
